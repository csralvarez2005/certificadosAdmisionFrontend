    ...
    this.estudianteService.obtenerNotasPorCodigo(this.estudiante.codigo)
      .subscribe({
        next: (notas) => {
          ...
          // 🔹 Filtrar duplicados por asignatura + notaDefinitiva
          const modulosUnicos = new Set<string>();
          notas = notas
            .filter(n => n.asignatura && n.notaDefinitiva !== null && n.notaDefinitiva !== undefined)
          ...
          ref.afterClosed().subscribe((result: { nivel: number; cuerpo: string }) => {
            if (result) {
              this.estudianteService
                .generarConstanciaNotasPersonalizada(this.estudiante.id, result.nivel, result.cuerpo)
                .subscribe((pdf: Blob) =>
                  this.descargarArchivo(pdf, `certificado_notas_${this.estudiante.codigo}_nivel${result.nivel}.pdf`)
                );
            }
          });
        },
...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content>
      ...
      <p class="info-programa">
        Duración de programa: 2 años<br>
        Intensidad horaria del programa: 1.358 horas<br>
    ...
    <mat-dialog-actions align="end">
      ...
      <button mat-button color="primary" (click)="ref.close({ nivel: nivelSeleccionado, cuerpo: cuerpoTexto })">Generar</button>
    </mat-dialog-actions>
  `,
  styles: [`
    .w-full { width: 100%; }
